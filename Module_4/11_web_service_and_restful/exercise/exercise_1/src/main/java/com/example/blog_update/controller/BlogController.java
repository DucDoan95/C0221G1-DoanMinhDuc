...
public class BlogController {
    ...
    public ResponseEntity<Page<Blog>> getListBlog(@PathVariable int page) {
        Page<Blog> blogList = iBlogService.findByAll(PageRequest.of(page-1,2));
        if (blogList.isEmpty()) {
    ...
    public ResponseEntity<Page<Blog>> getBlogByCategory(@PathVariable String name,@RequestParam Integer page){
        Page<Blog> blogList = iBlogService.findByCategory(name,PageRequest.of(page-1,2));
        if(blogList.isEmpty()){
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        }
        return new ResponseEntity<>(blogList,HttpStatus.OK);
